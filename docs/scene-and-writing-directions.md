...
Start with the mountain tarn for the biggest compositional change. The woodland lookout is the strongest concept to retain from Copilot. Give each scene one focal structure and one occasional animated event. Compose the phone crop deliberately. Once there are more than two scenes, replace the cycling button with a small named scene picker so visitors can choose directly.
