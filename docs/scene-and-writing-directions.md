...
Implemented the winter cabin near Braemar in the Cairngorms. A snow-laden log cabin with warm windows, a lantern and a smoking chimney stands on the left between two bare silver birches, with two rows of tiered, snow-laden pines and white ridges beyond under a low, pale sun. Sculpted drifts carry lit crests and shaded lee faces, a half-buried fence leads in, and footprints run to the door. It snows steadily, and every so often the fall thickens as larger flakes drift in, then eases again. By night the snow turns blue under the moon, the windows and lantern throw warm light onto it, and stars come out. The light-theme caption is slate blue so it stays readable against the snow. On phones the scene lifts and shifts so the cabin sits above the notes. The cycle is now Meadow → Coast → Blea Tarn → Kielder Forest → Merthyr Mawr → Manchester canal → Braemar. Coordinates rounded from https://en.wikipedia.org/wiki/Braemar (57.006° N, 3.399° W).
